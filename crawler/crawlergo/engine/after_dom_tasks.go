...
func (tab *Tab) AfterDOMRun() {
    ...
    tab.domWG.Wait()
    // logging.Logger.Debug("afterDOMRun end")
    tab.WG.Add(1)
    go tab.AfterLoadedRun()
}

// getBodyNodeId 获取的Body的NodeId 用于之后子节点无等待查询  最多等待3秒 如果DOM依旧没有渲染完成，则退出
func (tab *Tab) getBodyNodeId() bool {
    ...
    if len(docNodeIDs) == 0 || err != nil {
        // not root node yet?
        // logging.Logger.Debug("getBodyNodeId failed, maybe DOM not ready?")
        if err != nil {
...
func (tab *Tab) fillForm() {
    ...
    // logging.Logger.Debug("fillForm start")
...
// setObserverJS 设置Dom节点变化的观察函数
func (tab *Tab) setObserverJS() {
    defer tab.domWG.Done()
    // logging.Logger.Debug("setObserverJS start")
    // 设置Dom节点变化的观察函数
    go tab.Evaluate(js.ObserverJS)
    // logging.Logger.Debug("setObserverJS end")
}
...
func (f *FillForm) fillInput() {
    ...
    if inputErr != nil || len(inputNodes) == 0 {
        // logging.Logger.Debug("fillInput: get form input element err")
        if inputErr != nil {
...
func (f *FillForm) fillTextarea() {
    ...
    if textareaErr != nil || len(textareaNodes) == 0 {
        ...
        if textareaErr != nil {
            logging.Logger.Debug(textareaErr)
        }
        return
    }
...
func (f *FillForm) fillMultiSelect() {
    ...
    if optionErr != nil || len(optionNodes) == 0 {
        // logging.Logger.Debug("fillMultiSelect: get select option element err")
        if optionErr != nil {
            logging.Logger.Debug(optionErr)
        }
        return
    }
