...
func (tab *Tab) AfterLoadedRun() {
    defer tab.WG.Done()
    // logging.Logger.Debug("afterLoadedRun start")
...
func (tab *Tab) setFormToFrame() {
    ...
    if formErr != nil || len(formNodes) == 0 {
        // logging.Logger.Debug("setFormToFrame: get form element err")
        if formErr != nil {
            // logging.Logger.Debug(formErr)
        }
        return
    }
...
func (tab *Tab) clickSubmit() {
    ...
    formNodes, formErr := tab.GetNodeIDs(`form`)
    if formErr != nil || len(formNodes) == 0 {
        // logging.Logger.Debug("clickSubmit: get form element err")
        if formErr != nil {
            logging.Logger.Debug(formErr)
        }
        return
    ...
    if inputErr != nil || len(inputNodes) == 0 {
        // logging.Logger.Debug("clickSubmit: get form input element err")
        if inputErr != nil {
            logging.Logger.Debug(inputErr)
        }
        return
    }
...
func (tab *Tab) clickAllButton() {
    ...
    if bErr != nil || len(btnNodeIDs) == 0 {
        // logging.Logger.Debug("clickAllButton: get form button element err")
        if bErr != nil {
            logging.Logger.Debug(bErr)
        }
        return
    }
...
func (tab *Tab) triggerJavascriptProtocol() {
    defer tab.loadedWG.Done()
    // logging.Logger.Debug("clickATagJavascriptProtocol start")
    ...
    // logging.Logger.Debug("clickATagJavascriptProtocol end")
}
...
func (tab *Tab) RemoveDOMListener() {
    ...
    // logging.Logger.Debug("RemoveDOMListener start")
    // 移除DOM节点变化监听
    tab.Evaluate(js.RemoveDOMListenerJS)
    // logging.Logger.Debug("RemoveDOMListener end")
}
