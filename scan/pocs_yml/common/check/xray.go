...
import (
	...
	"github.com/yhy0/Jie/scan/pocs_yml/xray/cel"
	"github.com/yhy0/Jie/scan/pocs_yml/xray/requests"
...
func executeXrayPoc(oReq *http.Request, target string, poc *xray_structs.Poc) (isVul bool, err error, dump *Dump) {
	isVul = false
	...
	defer func() {
		if r := recover(); r != nil {
			logging.Logger.Errorf("%v Run Xray Poc [%s] error", r.(error), poc.Name)
			isVul = false
		}
	}()
	// 回收
	...
	logging.Logger.Debugf("Run Xray Poc[%s] for %s", poc.Name, target)

	// 判断transport，如果不合法则跳过
	transport := poc.Transport
	if transport == "tcp" || transport == "udp" {
		if strings.HasPrefix(target, "http://") || strings.HasPrefix(target, "https://") {
			logging.Logger.Errorf("Invalid target[%s], skip", target)
			return
		}
	} else {
	...
	if err != nil {
		logging.Logger.Errorln(err, "Environment creation error")
	...
	ReCreateEnv := func() error {
		env, err = cel.NewEnv(&c)
		if err != nil {
			logging.Logger.Errorf("Environment re-creation error: %v", err)
			return err
		}
		return nil
	}
	...
	evaluateUpdateVariableMap := func(set yaml.MapSlice) {
		for _, item := range set {
			...
			if err != nil {
				logging.Logger.Errorf("%v Evalaute expression error: %s", err, expression)
				continue
			}
	...
	HttpRequestInvoke := func(rule xray_structs.Rule) error {
		...
		if request, protoRequest, protoResponse, ok = requests.XrayGetHttpRequestCache(&ruleReq); !ok || !rule.Request.Cache {
			...
			if err != nil {
				logging.Logger.Errorf("%v Run poc[%v] parse request error", err, poc.Name)
				return err
			}
			...
			if err != nil {
				logging.Logger.Errorf("%v Run poc[%s] parse response error", err, poc.Name)
				return err
			}
			...
			logging.Logger.Debugf("Hit http request cache[%s%s]", oReqUrlString, ruleReq.Path)
		}
	...
	TCPUDPRequestInvoke := func(rule xray_structs.Rule) error {
		...
		if responseRaw, protoResponse, ok = requests.XrayGetTcpUdpResponseCache(rule.Request.Content); !ok || !rule.Request.Cache {
			...
			if connCache, ok = requests.XrayGetTcpUdpConnectionCache(connectionID); !ok {
				...
				if err != nil {
					logging.Logger.Errorf("%v Parse read_timeout[%s] to int  error", err, rule.Request.ReadTimeout)
					return err
				}
				...
				if err != nil {
					logging.Logger.Errorf("%v %s connect to target[%s] error", err, tcpudpTypeUpper, target)
					return err
				}
				...
				if err != nil {
					logging.Logger.Errorf("%v %v Set read_timeout [%d] error", err, tcpudpTypeUpper, readTimeout)
					return err
				}
				...
				conn = *connCache
				logging.Logger.Debugf("Hit connection_id cache[%s]", connectionID)
			}
			...
			if err != nil {
				logging.Logger.Errorf("%v %s[%s] write error", err, tcpudpTypeUpper, connectionID)
				return err
			}
			...
			for {
				...
				if err != nil {
					if err == io.EOF {
						...
						logging.Logger.Errorf("%v %s[%s] read error", err, tcpudpTypeUpper, connectionID)
						return err
					}
					break
				}
			...
			logging.Logger.Debugf("Hit tcp/udp request cache[%s]", responseRaw)
		}
	...
	RequestInvoke := func(requestFunc cel.RequestFuncType, ruleName string, rule xray_structs.Rule) (bool, error) {
		...
		logging.Logger.Debugf("raw requests: \n%#s", string(protoRequest.Raw))
		logging.Logger.Debugf("raw response: \n%#s", string(protoResponse.Raw))

		// 执行表达式
		out, err := cel.Evaluate(env, rule.Expression, variableMap)

		if err != nil {
			logging.Logger.Errorf("%v Evalute rule[%s] expression error: %s", err, ruleName, rule.Expression)
			return false, err
		}
	...
	run := func() (bool, error) {
		...
		if err != nil {
			logging.Logger.Errorf("%v Evalute poc[%s] expression error: %s", err, poc.Name, poc.Expression)
			return false, err
		}
	...
	dump = &Dump{
		Req: string(protoRequest.Raw),
		Res: string(protoResponse.Raw),
	}
