...
func (tab *Tab) InterceptRequest(v *fetch.EventRequestPaused) {
    ...
    if err != nil {
        // logging.Logger.Debug("InterceptRequest parse url failed: ", err)
...
func (tab *Tab) HandleAuthRequired(req *fetch.EventAuthRequired) {
    defer tab.WG.Done()
    // logging.Logger.Debug("auth required found, auto auth.")
    ctx := tab.GetExecutor()
...
func (tab *Tab) HandleNavigationReq(req *model.Request, v *fetch.EventRequestPaused) {
    ...
    if tab.FoundRedirection && tab.IsTopFrame(v.FrameID.String()) {
        ...
        // logging.Logger.Debug("has redirection_flag: " + req.URL.String())
...
func (tab *Tab) HandleRedirectionResp(v *network.EventResponseReceivedExtraInfo) {
    ...
    if 300 <= statusCode && statusCode < 400 {
        // logging.Logger.Debug("set redirect flag.")
