		...
		cel.Functions(
			...
			&functions.Overload{
				...
				Unary: func(value ref.Val) ref.Val {
					...
					return types.String(util.RandLowLetterNumber(int(n)))
				},
			},
			&functions.Overload{
			...
			&functions.Overload{
				...
				Unary: func(value ref.Val) ref.Val {
					...
					return types.Int(util.Mmh3Hash32(util.Base64Encode(b)))
				},
			},
			&functions.Overload{
...
// xray dns反连平台 目前只支持 dnslog.cn 和 ceye.io
func xrayNewReverse() (reverse *structs.Reverse) {
	...
	case structs.ReverseType_Ceye:
		sub := util.RandLowLetterNumber(8)
	...
	case structs.ReverseType_DnslogCN:
		...
		if err != nil {
			logging.Logger.Errorln(err, "Get reverse domain error: Can't get domain from dnslog.cn")
			return
		}
	...
	logging.Logger.Debugf("Get reverse domain: %s", u.Hostname())
...
func reverseCheck(r *structs.Reverse, timeout int64) bool {
	...
	case structs.ReverseType_Ceye:
		...
		if err != nil {
			logging.Logger.Errorln(err, "Reverse check error")
			return false
		}
		content, _ := requests.GetRespBody(resp)

		if !bytes.Contains(content, []byte(`"data": []`)) { // api返回结果不为空
			logging.Logger.Debugf("Got reverse dnslog from %s", r.Domain)
			return true
		}
		return false
	case structs.ReverseType_DnslogCN:
