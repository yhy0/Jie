	...
	chromedp.ListenTarget(tab.Ctx, func(e interface{}) {
		...
		case *fetch.EventRequestPaused:
			...
			if v.ResourceType == network.ResourceTypeImage || url.FileExt() == "ico" {
				...
				_ = fetch.FulfillRequest(v.RequestID, 200).
					WithBody(data).
				...
				return
			} else if cconfig.StaticSuffixSet.Contains(url.FileExt()) {
				// 静态资源 全部阻断
			...
			//	defer tab.WG.Done()
			//	//logging.Logger.Debugf("[dom-based] EventRequestPaused: %s %d\n", v.Request.URL, v.ResponseStatusCode)
			//	if err := xss.ParseDomHookResponseJs(tab.GetExecutor(), v); err != nil {
		...
		case *network.EventResponseReceived:
			if v.Response.MimeType == "application/javascript" || v.Response.MimeType == "text/html" || v.Response.MimeType == "application/json" {
				tab.WG.Add(1)
				go tab.ParseResponseURL(navigateReq.URL.String(), v)
			}
		...
		//	logging.Logger.Error("EventLoadingFailed ", v.ErrorText)
		...
		case *runtime.EventBindingCalled:
			switch v.Name {
			case xss.EventPushVul:
				points := make([]xss.VulPoint, 0)
				if err := json.Unmarshal([]byte(v.Payload), &points); err != nil {
...
func (tab *Tab) Start() {
	//logging.Logger.Info("Crawling " + tab.NavigateReq.Method + " " + tab.NavigateReq.URL.String())
	defer tab.Cancel()
...
func (tab *Tab) HandleBindingCalled(event *runtime.EventBindingCalled) {
	...
	if bcPayload.Name == "addLink" && len(bcPayload.Args) > 1 {
		tab.AddResultUrl(cconfig.GET, bcPayload.Args[0], bcPayload.Args[1])
	}
...
func (tab *Tab) Evaluate(expression string) {
	...
	if exception != nil {
		logging.Logger.Debug("tab Evaluate: ", exception.Text)
	}
	if err != nil {
		logging.Logger.Debug("tab Evaluate: ", err)
	}
}
...
func IsIgnoredByKeywordMatch(req model.Request, IgnoreKeywords []string) bool {
	for _, _str := range IgnoreKeywords {
		if strings.Contains(req.URL.String(), _str) {
			logging.Logger.Info("ignore request: ", req.SimpleFormat())
			return true
		}
	}
	return false
