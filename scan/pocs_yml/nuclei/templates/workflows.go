...
func compileWorkflow(path string, preprocessor Preprocessor, options *protocols.ExecuterOptions, workflow *workflows.Workflow, loader model.WorkflowLoader, pocs embed.FS) {
	for _, workflow := range workflow.Workflows {
		if err := parseWorkflow(preprocessor, workflow, options, loader, pocs); err != nil {
...
func parseWorkflow(preprocessor Preprocessor, workflow *workflows.WorkflowTemplate, options *protocols.ExecuterOptions, loader model.WorkflowLoader, pocs embed.FS) error {
	...
	if err := parseWorkflowTemplate(workflow, preprocessor, options, loader, shouldNotValidate, pocs); err != nil {
		return err
	}
	for _, subtemplates := range workflow.Subtemplates {
		if err := parseWorkflow(preprocessor, subtemplates, options, loader, pocs); err != nil {
...
func parseWorkflowTemplate(workflow *workflows.WorkflowTemplate, preprocessor Preprocessor, options *protocols.ExecuterOptions, loader model.WorkflowLoader, noValidate bool, pocs embed.FS) error {
	var paths []string
	...
	for _, path := range paths {
		template, err := Parse(path, preprocessor, options.Copy(), pocs)
		if err != nil {
