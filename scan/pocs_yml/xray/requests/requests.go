...
func InitHttpClient(ThreadsNum int, DownProxy string, timeout int) error {
	dialer := &net.Dialer{
		Timeout:   DialTimout,
		KeepAlive: KeepAlive,
	}
	...
	if DownProxy != "" {
		...
		if err != nil {
			logging.Logger.Errorf("Parse Proxy error: %v", err)
			return err
		}
...
func DoRequest(req *http.Request, redirect bool) (*http.Response, int64, error) {
	...
	if err != nil {
		logging.Logger.Errorf("Request error: %v", err)
		return nil, 0, err
	}
...
func ParseHttpRequest(oReq *http.Request) (*structs.Request, error) {
	...
	if oReq.Body != nil && oReq.Body != http.NoBody {
		...
		if err != nil {
			logging.Logger.Errorf("Get request error: %v", err)
			return nil, err
		}
		req.Body = data
...
func GetRespBody(oResp *http.Response) ([]byte, error) {
	...
	if oResp.Header.Get("Content-Encoding") == "gzip" {
		...
		if err != nil {
			logging.Logger.Errorf("Get response body error: %v", err)
			return nil, err
		}
		body = raw
	}
