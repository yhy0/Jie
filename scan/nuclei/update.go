...
func (r *Runner) freshTemplateInstallation(configDir string, ctx context.Context) error {
	logging.Logger.Infoln("nuclei-templates are not installed, installing...")
	...
	logging.Logger.Infof("Downloading nuclei-templates (v%s) to %s", version.String(), r.templatesConfig.TemplatesDirectory)
	...
	logging.Logger.Infof("Successfully downloaded nuclei-templates (v%s) to %s. GoodLuck!", version.String(), r.templatesConfig.TemplatesDirectory)
...
func (r *Runner) updateTemplatesWithVersion(latestVersion semver.Version, currentVersion semver.Version, runner *Runner, ctx context.Context) error {
	if latestVersion.GT(currentVersion) {
		logging.Logger.Infof("Your current nuclei-templates v%s are outdated. Latest is v%s", currentVersion, latestVersion.String())
		logging.Logger.Infoln("Downloading latest release...")
		...
		logging.Logger.Infof("Downloading nuclei-templates (v%s) to %s", latestVersion.String(), runner.templatesConfig.TemplatesDirectory)
		...
		logging.Logger.Infof("Successfully updated nuclei-templates (v%s) to %s. GoodLuck!", latestVersion.String(), r.templatesConfig.TemplatesDirectory)
	}
	return nil
}
...
func (r *Runner) checkNucleiIgnoreFileUpdates(configDir string) bool {
	...
	if r.templatesConfig != nil {
		if err := config.WriteConfiguration(r.templatesConfig); err != nil {
			logging.Logger.Infof("Could not get ignore-file from server: %s", err)
		}
	}
	return true
}
...
func (r *Runner) printUpdateChangelog(results *templateUpdateResults, version string) {
	if len(results.additions) > 0 && r.options.Verbose {
		logging.Logger.Infoln("Newly added templates: ")
		for _, addition := range results.additions {
			logging.Logger.Infof("%s", addition)
		}
	}

	logging.Logger.Infof("Nuclei Templates v%s Changelog", version)
	data := [][]string{
...
func (r *Runner) fetchLatestVersionsFromGithub(configDir string) {
	...
	if err != nil {
		logging.Logger.Warnf("Could not fetch latest releases: %s", err)
		return
	}
	if r.templatesConfig != nil {
