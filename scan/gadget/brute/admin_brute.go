...
func getinput(inputurl string, client *httpx.Client) (usernamekey string, passwordkey string, loginurl string, ismd5 bool) {
    usernamekey = "username"
    passwordkey = "password"
    client.Options.AllowRedirect = 5
    if req, err := client.Request(inputurl, "GET", "", nil); err == nil {
        ...
        if hreflist != nil {
            ...
            req, err = client.Request(hrefurl.String(), "GET", "", nil)
            if err != nil {
                return "", "", "", false
            }
        }
...
func Admin_brute(u string, client *httpx.Client) (username string, password string, loginurl string) {
    if SkipAdminBrute {
        return "", "", ""
    }
    usernamekey, passwordkey, loginurl, ismd5 := getinput(u, client)
    var (
        ...
        lockContent           = []string{"锁定", "次数超", "超次数", "验证码错误", "请输入验证码", "请输入正确的验证码", "验证码不能为空", "\\u9501\\u5b9a", "\\u6b21\\u6570\\u8d85", "\\u8d85\\u6b21\\u6570", "\\u9a8c\\u8bc1\\u7801\\u9519\\u8bef", "\\u8bf7\\u8f93\\u5165\\u9a8c\\u8bc1\\u7801", "\\u8bf7\\u8f93\\u5165\\u6b63\\u786e\\u7684\\u9a8c\\u8bc1\\u7801", "\\u9a8c\\u8bc1\\u7801\\u4e0d\\u80fd\\u4e3a\\u7a7a", "exmail.qq.com/cgi-bin/login"}
    ...
    if adminfalseurl, err := client.Request(loginurl, "POST", adminfalsedata, nil); err == nil {
        ...
        case 401:
            falseis401 = true
        case 200:
            if util.SliceInCaseFold(adminfalseurl.Body, noaccount) {
                adminaccount = false
            }
            falseis200 = true
    ...
    if testfalseurl, err := client.Request(loginurl, "POST", testfalsedata, nil); err == nil {
        ...
        case 401:
            falseis401 = true
        case 200:
            if util.SliceInCaseFold(testfalseurl.Body, noaccount) {
                testaccount = false
            }
            falseis200 = true
    ...
    for _, user := range usernames {
        for _, pass := range top100pass {
            ...
            if req, err2 := client.Request(loginurl, "POST", fmt.Sprintf("%s=%s&%s=%s", usernamekey, user, passwordkey, pass), nil); err2 == nil {
                if falseis401 {
                ...
                if falseis302 {
                    ...
                    if req.Location != adminfalse302location && req.Location != testfalse302location {
                        ...
                        if sucesstest, err := client.Request(loginurl, "POST", sucesstestdata, nil); err == nil {
                ...
                if falseis200 {
                    if util.SliceInCaseFold(req.Body, lockContent) {
                        return "", "", ""
                    }
