...
func (s *Wappalyze) Fingerprint(headers map[string][]string, body []byte) map[string]struct{} {
    ...
    fingerprintHub := s.Identify(headers, body)
    if fingerprintHub != "" {
        uniqueFingerprints.setIfNotExists(fingerprintHub)
    }
    if Honeypot(string(body)) {
        uniqueFingerprints.setIfNotExists("Honeypot")
    }
...
func (s *Wappalyze) FingerprintWithTitle(headers map[string][]string, body []byte) (map[string]struct{}, string) {
    ...
    fpHub := s.Identify(headers, body)
    if fpHub != "" {
        uniqueFingerprints.setIfNotExists(fpHub)
    }
    if Honeypot(string(body)) {
        uniqueFingerprints.setIfNotExists("Honeypot")
    }
