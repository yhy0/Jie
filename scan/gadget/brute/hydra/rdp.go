...
func (g *Client) Login(domain, user, pwd string, timeout int64) error {
    rdpConn, err := net.DialTimeout("tcp", g.Host, time.Duration(timeout)*time.Second)
    defer func() {
        if rdpConn != nil {
            rdpConn.Close()
        }
    ...
    glog.Info(rdpConn.LocalAddr().String())

    g.tpkt = tpkt.New(core.NewSocketLayer(rdpConn), nla.NewNTLMv2(domain, user, pwd))
