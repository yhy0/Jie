...
import (
	...
	weblogic2 "github.com/yhy0/Jie/scan/java/weblogic"
	seeyon2 "github.com/yhy0/Jie/scan/oa/seeyon"
	"github.com/yhy0/Jie/scan/oa/yongyou/nc"
	"github.com/yhy0/Jie/scan/pocs_go/ThinkPHP"
	"github.com/yhy0/Jie/scan/pocs_go/fastjson"
	"github.com/yhy0/Jie/scan/pocs_go/jboss"
	"github.com/yhy0/Jie/scan/pocs_go/jenkins"
	"github.com/yhy0/Jie/scan/pocs_go/phpunit"
	"github.com/yhy0/Jie/scan/pocs_go/tomcat"
	"net/url"
	"time"
)
...
func PocCheck(technologies []string, target string, finalURL string, ip string) {
	...
	for _, wt := range technologies {
		...
		switch tech {
		case "Shiro":
			key, mode := shiro.CVE_2016_4437(finalURL, "")
			if key != "" {
				vulnerability = true
				plugin = "Shiro"
				payload = mode + ": " + key
		...
		case "Apache Tomcat":
			username, password := brute.TomcatBrute(target)
			if username != "" {
				vulnerability = true
			...
			if tomcat.CVE_2017_12615(target) {
				vulnerability = true
				plugin = "Apache Tomcat"
		...
		case "Basic": // todo 这里还没有匹配到
			username, password, _ := brute.BasicBrute(target)
			if username != "" {
				vulnerability = true
				plugin = "Basic"
		...
		case "WebLogic":
			username, password := brute.WeblogicBrute(target)
			if username != "" {
				vulnerability = true
				plugin = "WebLogic"
		...
		case "Jboss", "Jboss application server 7", "Jboss-as", "Jboss-eap", "Jboss web", "Jboss application server":
			...
			username, password := brute.JbossBrute(target)
			if username != "" {
				vulnerability = true
				plugin = "Jboss"
		...
		case "Jenkins":
			if jenkins.Unauthorized(target) {
				vulnerability = true
				plugin = "Jenkins"
				payload += "exp-Jenkins|Unauthorized script \n"
			}
			if jenkins.CVE_2018_1000110(target) {
				vulnerability = true
				plugin = "Jenkins"
				payload += "exp-Jenkins|CVE_2018_1000110 \n"
			}
			if jenkins.CVE_2018_1000861(target) {
				vulnerability = true
				plugin = "Jenkins"
				payload += "exp-Jenkins|CVE_2018_1000861 \n"
			}
			if jenkins.CVE_2019_10003000(target) {
				vulnerability = true
				plugin = "Jenkins"
