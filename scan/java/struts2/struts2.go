...
func S2(options conf.Options) {
	if options.S2.Mode == "scan" {
		...
		case "s2-001":
			if options.S2.Body != "" {
				s2_001.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s001须指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body \"user=a&pass=fuckit\"")
			}
		case "s2-005":
			s2_005.Check(options.Target)
		case "s2-007":
			if options.S2.Body != "" {
				s2_007.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s007需指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body \"user=a&pass=fuckit\"")
			}
		case "s2-008":
			s2_008.Check(options.Target)
		case "s2-009":
			if options.S2.Body != "" {
				s2_009.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s009需指定要测试的GET参数，如: --options.S2.Body=\"name\"")
			}
		case "s2-012":
			if options.S2.Body != "" {
				s2_012.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s012需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body \"user=a&pass=fuckit\"")
			}
		case "s2-013":
			s2_013.Check(options.Target)
		case "s2-015":
			s2_015.Check(options.Target)
		case "s2-016":
			s2_016.Check(options.Target)
		case "s2-045":
			s2_045.Check(options.Target)
		case "s2-046":
			s2_046.Check(options.Target)
		case "s2-048":
			if options.S2.Body != "" {
				s2_048.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s048需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body \"user=a&pass=fuckit\"")
			}
		case "s2-053":
			if options.S2.Body != "" {
				s2_053.Check(options.Target, options.S2.Body)
			} else {
				fmt.Println("s053需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body \"user=a&pass=fuckit\"")
			}
		case "s2-057":
			s2_057.Check(options.Target)
		case "all":
		...
		case "s2-001":
			if options.S2.Body != "" {
				s2_001.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
			} else {
				fmt.Println("s001需手动指定POST数据包内容，并用fuckit标记出测试点，如: --options.S2.Body=\"user=a&pass=fuckit\"")
			}
		case "s2-005":
			s2_005.ExecCommand(options.Target, options.S2.CMD)
		case "s2-007":
			if options.S2.Body != "" {
				s2_007.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
			} else {
				fmt.Println("s007需手动指定POST数据包内容，并用fuckit标记出测试点，如: --options.S2.Body=\"user=a&pass=fuckit\"")
			}
		case "s2-008":
			s2_008.ExecCommand(options.Target, options.S2.CMD)
		case "s2-009":
			s2_009.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
		case "s2-012":
			if options.S2.Body != "" {
				s2_012.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
			} else {
				fmt.Println("s012需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body=\"user=a&pass=fuckit\"")
			}
		case "s2-013":
			s2_013.ExecCommand(options.Target, options.S2.CMD)
		case "s2-015":
			s2_015.ExecCommand(options.Target, options.S2.CMD)
		case "s2-016":
			s2_016.ExecCommand(options.Target, options.S2.CMD)
		case "s2-045":
			s2_045.ExecCommand(options.Target, options.S2.CMD)
		case "s2-046":
			s2_046.ExecCommand(options.Target, options.S2.CMD)
		case "s2-047":
			if options.S2.Body != "" {
				s2_048.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
			} else {
				fmt.Println("s048需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body=\"user=a&pass=fuckit\"")
			}
		case "s2-053":
			if options.S2.Body != "" {
				s2_053.ExecCommand(options.Target, options.S2.CMD, options.S2.Body)
			} else {
				fmt.Println("s053需手动指定POST数据包内容，并用<fuckit>标记出测试点，如: --options.S2.Body=\"user=a&pass=fuckit\"")
			}
		case "s2-057":
			s2_057.ExecCommand(options.Target, options.S2.CMD)
		case "all":
