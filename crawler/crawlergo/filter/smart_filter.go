...
type SmartFilter struct {
    StrictMode bool
    *SimpleFilter
    filterLocationSet          mapset.Set[string] // 非逻辑型参数的位置记录 全局统一标记过滤
    ...
    uniqueMarkedIds            mapset.Set[string] // 标记后的唯一ID，用于去重
}
...
func NewSmartFilter(base *SimpleFilter, strictMode bool) *SmartFilter {
    s := &SmartFilter{}
    s.filterLocationSet = mapset.NewSet[string]()
    ...
    s.uniqueMarkedIds = mapset.NewSet[string]()
    s.SimpleFilter = base
    s.StrictMode = strictMode
    return s
}

// DoFilter 智能去重 可选严格模式 需要过滤则返回 true
...
func (s *SmartFilter) calcFragmentID(fragment string) string {
    ...
    if err != nil {
        logging.Logger.Error("cannot calculate url fragment: ", err)
        return ""
    }
