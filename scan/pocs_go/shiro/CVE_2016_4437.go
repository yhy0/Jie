...
func getkeylen(u string, indexlen int, rememberMe string) (int, error) {
	...
	if req, err := http.Request(u, "GET", "", false, header); err == nil {
	...
	if indexlen, err := getkeylen(u, 0, "1"); err == nil {
		...
		for _, key = range keys {
			...
			if CBClen, err := getkeylen(u, indexlen, RememberMe1); err == nil {
				if CBClen != indexlen {
					logging.Logger.Infof(fmt.Sprintf("Found vuln Shiro CVE_2016_4437| URL: %s CBC-KEY: %s\n", u, key))
					return key
				}
			}
			if GCMlen, err := getkeylen(u, indexlen, RememberMe2); err == nil {
				if GCMlen != indexlen {
					logging.Logger.Infof(fmt.Sprintf("Found vuln Shiro CVE_2016_4437| URL: %s GCM-KEY: %s\n", u, key))
					return key
				}
			}
		}
