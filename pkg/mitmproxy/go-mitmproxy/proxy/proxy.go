...
func NewProxy(opts *Options) (*Proxy, error) {
    ...
    if opts.Header == "" {
        opts.Header = "Go-Mitmproxy-Authorization"
    }
...
func (proxy *Proxy) ServeHTTP(res http.ResponseWriter, req *http.Request) {
    ...
    // 这里增加一个认证判断
    if proxy.Opts.Username != "" && proxy.Opts.Password != "" {
        auth := req.Header.Get(proxy.Opts.Header)
        if auth == "" {
            res.WriteHeader(403)
            io.WriteString(res, "代理服务器认证失败")
            return
        }

        username, password, ok := ParseBasicAuth(auth)
        if !ok || username != proxy.Opts.Username || password != proxy.Opts.Password {
            res.WriteHeader(403)
            io.WriteString(res, "代理服务器认证失败")
            return
        } else {
            // 认证通过后删除该 header，防止污染请求或泄露信息
            req.Header.Del(proxy.Opts.Header)
        }
    }
