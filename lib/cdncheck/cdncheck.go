...
func (c *Client) CheckDomainWithFallback(domain string) (matched bool, value string, itemType string, err error, dnsData *retryabledns.DNSData) {
    ...
    if err != nil {
        return false, "", "", err, dnsData
    }
    matched, value, itemType, err, _ = c.CheckDNSResponse(dnsData)
    if err != nil {
        return false, "", "", err, dnsData
    }
    if matched {
        return matched, value, itemType, nil, dnsData
    }
    // resolve cname
...
func (c *Client) CheckDNSResponse(dnsResponse *retryabledns.DNSData) (matched bool, value string, itemType string, err error, dnsData *retryabledns.DNSData) {
    if dnsResponse.A != nil {
        for _, ip := range dnsResponse.A {
            ...
            if err != nil {
                return false, "", "", err, dnsResponse
            }
            if matched {
                return matched, value, itemType, nil, dnsResponse
            }
        }
    }
    if dnsResponse.CNAME != nil {
        ...
        if err != nil {
            return false, "", itemType, err, dnsResponse
        }
        if matched {
            // for now checkSuffix only checks for wafs
            return matched, discovered, itemType, nil, dnsResponse
        }
    }
    return false, "", "", err, dnsResponse
}
